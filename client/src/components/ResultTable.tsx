...
import { AddResultForm, UpdateStudentForm } from "./AllComponents";
import { ResultType } from "../TypeHints";

const columns = [
  { name: "RESULT ID", uid: "id", sortable: true },
  { name: "EXAM ID", uid: "exam_id", sortable: true },
  { name: "COURSE ID", uid: "course_id", sortable: true },
  { name: "STUDENT ID", uid: "student_id", sortable: true },
  { name: "FIRST NAME", uid: "student_first_name", sortable: true },
  { name: "LAST NAME", uid: "student_last_name", sortable: true },
  { name: "EXAM DATE", uid: "exam_date", sortable: true },
  { name: "COURSE NAME", uid: "course_name", sortable: true },
  { name: "MARKS OBTAINED", uid: "marks_obtained", sortable: true },
...
export default function ResultTable() {
  // All students
  const [users, setUsers] = React.useState<ResultType[]>([
    {
      id: "",
      student_id: "",
      student_first_name: "",
      ...
      <div className="py-2 px-2 flex justify-between items-center">
        <span className="w-[30%] text-small text-default-400">
          ...
          {filteredItems.length !== users.length &&
            `After search and filter ${filteredItems.length} results in total.`}
        </span>
        <Pagination
          isCompact
          showControls
  ...
  const handleDeleteUser = () => {
    async function deleteUser() {
      await axios
        .delete(`${baseUrl}/results/delete`, {
          params: { id: currentUser.id },
        })
        .then((response) => {
          toast.success(
            response?.data?.message || "Result deleted successfully",
          );
        })
        .catch((err) => {
          toast.error(
            err?.response?.data?.message || "Unable to delete the result",
          );
        });
    }
    deleteUser();
                  ...
                  <ModalBody>
                    <div className="grid grid-cols-12 gap-4">
                      ...
                      <Input
                        ...
                        label="Exam Date"
                        type="date"
                        defaultValue={currentUser.exam_date}
                        className="col-span-3"
                      />
                      <Input
                        isReadOnly
                        ...
                        type="number"
                        defaultValue={currentUser.marks_obtained}
                        className="col-span-3"
                      />
  ...
  useEffect(() => {
    axios
      .get(`${baseUrl}/results/all`, {
        params: { search_query: null, page: 1, per_page: 20 },
      })
      .then((response) => {
