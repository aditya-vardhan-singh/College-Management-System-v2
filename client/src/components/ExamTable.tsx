...
import { AddExamForm, UpdateStudentForm } from "./AllComponents";
import { ExamType } from "../TypeHints";
...
const columns = [
  ...
  { name: "EXAM TYPE", uid: "exam_type", sortable: true },
  { name: "MAX MARKS", uid: "max_marks", sortable: true },
...
export default function ExamTable() {
  // All students
  const [exams, setExams] = React.useState<ExamType[]>([
    {
      id: "",
      course_id: "",
      course: "",
  ...
  const filteredItems = React.useMemo(() => {
    let filteredUsers = [...exams];

    if (hasSearchFilter) {
      filteredUsers = filteredUsers.filter((user) =>
        (user.course + user.exam_date + user.exam_type + user.max_marks)
          .toLowerCase()
  ...
  }, [exams, filterValue, statusFilter, hasSearchFilter]);
  ...
  const [currentUser, setCurrentUser] = useState(exams[0]);
    ...
    return (
      <div className="py-2 px-2 flex justify-between items-center">
        <span className="w-[30%] text-small text-default-400">
          ...
          {filteredItems.length !== exams.length &&
  ...
  const handleDeleteUser = () => {
    async function deleteUser() {
      await axios
        ...
        .catch((err) => {
          toast.error(err?.response?.data?.message || "Unable to delete exam");
        });
    }
    deleteUser();
  };
  ...
  useEffect(() => {
    axios
      ...
      .catch((err) => {
        ...
        if (isAxiosError(err)) {
          toast.error(
            err?.response?.data?.message || "Error getting exam records!",
          );
        } else {
  ...
  return (
    <>
      ...
      >
        ...
        <TableBody emptyContent={"No exam records found"} items={sortedItems}>
          {(item) => (
